...
class QuestionMatching(nn.Layer):
    def __init__(self, pretrained_model, dropout=None, rdrop_coef=0.0, gru_hidden_size=192):
        ...
        # Max pooling and average pooling parameters
    ...
    def forward(self,
        ...
        sentence_embedding, cls_embedding1 = self.ptm(input_ids, token_type_ids, position_ids, attention_mask)
        # sentence_embedding size: batch_size, sequence_length, hidden_size
        # cls_embedding size: batch_size, hidden_size
        # cls_embedding goes through the fully connected layer
        cls_out = self.Linear(cls_embedding1)
        cls_out = self.dropout(cls_out)
        # sentence_embedding goes through GRU
        gru_out, (last_hidden, _) = self.gru_layer(sentence_embedding)
        # sentence_embedding goes through max pooling layer
        ...
        # sentence_embedding goes through average pooling layer
